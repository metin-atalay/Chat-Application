...
func createRecentItems(chatRoomId : String,users : [User])  {
    ...
    var memberIdsToCreteRecent = [users.first!.id,users.last!.id]

    print("members to create recent is ", memberIdsToCreteRecent)
    ...
    FirebaseReference(.Recent).whereField(kCHATROOMID, isEqualTo: chatRoomId).getDocuments { (snapshot, error) in
        ...
        if !snapshot.isEmpty {
            ...
            memberIdsToCreteRecent = removeMemberWhohasRecent(snapshot: snapshot, memberIds: memberIdsToCreteRecent)
            
            print("updated to create recent is ", memberIdsToCreteRecent)
            
        }
        
        for userId in memberIdsToCreteRecent {
            
            print("creating rencet for user with id ", memberIdsToCreteRecent)
            ...
            let recentObject = RecentChat(id: UUID().uuidString, chatRoomId: chatRoomId, senderId: senderUser.id, senderName: senderUser.username, receiverId: receiverUser.id, receiverName: receiverUser.username, date: Date(), memberIds: [senderUser.id,receiverUser.id], lastMessage: "SLM", unreadCounter: 0, avatarLink: receiverUser.avatarLink)
...
func removeMemberWhohasRecent(snapshot: QuerySnapshot, memberIds: [String]) -> [String] {
    ...
    for recentData in snapshot.documents {
        let currenctRecent = recentData.data() as Dictionary
        
        if let currenctUserId = currenctRecent[kSENDERID] {
            
            if memberIdsToCreateRecent.contains(currenctUserId as! String) {
                
                
                memberIdsToCreateRecent.remove(at: memberIdsToCreateRecent.firstIndex(of: currenctUserId as! String)!)
            }
        }
    }
